...
class QKGemmaNormRopeFusionPattern(BasePattern):
    def __init__(self, vllm_config, head_dim, num_heads, num_kv_heads, eps=1e-6):
        super().__init__(vllm_config, eps)
        self.head_dim = head_dim
        self.num_heads = num_heads
    ...
    def get_pattern(self):
        def pattern(
            ...
            q_by_head = q.view(*q.shape[:-1], q.shape[-1] // self.head_dim, self.head_dim)
            q_norm_out, _ = torch.ops.npu.npu_gemma_rms_norm(q_by_head.clone(), q_weight, self.eps)

            k_by_head = k.view(*k.shape[:-1], k.shape[-1] // self.head_dim, self.head_dim)
            k_norm_out, _ = torch.ops.npu.npu_gemma_rms_norm(k_by_head.clone(), k_weight, self.eps)
...
class QKNormRopeFusionPass(VllmInductorPass):
    ...
    def __init__(self, vllm_config: VllmConfig):
        ...
        for layer in attn_layers:
            pass_configs.add((layer.head_size, layer.num_heads, layer.num_kv_heads))
        
        for head_size, num_heads, num_kv_heads in pass_configs:
    ...
    def __call__(self, graph: torch.fx.Graph):
        self.begin()
        print(f"{graph.graph=}")
        logger.info("Fused %s QKNorm and Rope patterns", self.matched_count)
        logger.info("Patterns registered for replacement:")
        pattern_idx = 0
